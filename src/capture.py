"""
Capture screenshot TradingView — layout Ichimoku (session Playwright).

Prérequis : secrets/storage_state.json exporté depuis Windows (Playwright).
"""
...
from src.settings import AppConfig, ensure_playwright_browsers_path, load_settings

LOGIN_URL_FRAGMENT = "/accounts/signin"

# Délai fixe post-navigation (aligné sur tv_capture.py — pas de wait_for_selector).
...
def _build_output_path(cfg: AppConfig) -> Path:
    """Nom horodaté : BTCUSDT_4h_YYYYMMDD_HHMMSS.png"""
...
def capture_chart(cfg: AppConfig | None = None, wait_ms: int | None = None) -> Path:
    """
